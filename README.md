...
```bash
docker run -d -p 9384:9384 jmal98/sqs-exporter:0.0.3
```
...
```bash
docker run -d -p 9384:9384 -e AWS_ACCESS_KEY_ID=<access key> -e AWS_SECRET_ACCESS_KEY=<secret key> -e AWS_REGION=<region>  jmal98/sqs-exporter:0.0.3
```
